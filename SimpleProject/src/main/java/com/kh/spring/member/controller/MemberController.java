...
import lombok.extern.slf4j.Slf4j;
	...
	public class MemberController {
	 ...
	 //로그아웃 요청 처리할 handler를 만든다
	  ...
	  public String joinForm() {
		  //포워딩 할 JSP파일의 논리적인 경로
		  //WEB-INF/views/member/signUp.jsp
		  return"member/signUp";
		  //join을 누르면 아이디 비번을 입력하는 창이나오고 
		  //그리고 회원 가입을 누르면 회원가입이이 완료 되는 창으로 또 넘어가야 하니까 그게 signUp
	  }
	  // 사용자가 입력한 값을 받아줘야지
	  //값뽑기
	  //가공
	  //하기위해서 매개변수 자리에 MemberDTO를 넣어주겠따
	 @PostMapping("signUp")
	 //인코디을 해야 한글이 깨지지 않고 잘나옴
	 //디스패처 가기전에 filter에서 인코딩을 해야 바뀌어서 넘어간다.
	...
	public String myPage() {
		//ModelAndView 커트롤러에서 뷰로 전달하는것이다
		return "member/my_page";
	}
	@PostMapping("edit")
	public String edit(MemberDTO member,HttpSession session) {
		/*404 : mapping값 확인하기
		 * org.springframwork.web.servlet.pageNotFound
		 * 
		 * 405 : mapping값 잘 씀 type을 GET/POST를 잘못 적어서
		 * 
		 * 필드에 값이 잘들어왔나 ? =key값 확인
		 */
		log.info("값 찍어보기 {}",member);
		/*2.SQL문 
		UPDATE => MEMBER => PK?
		ID PWD NAME EMAIL ENROLLDATE
		2-1매개변수 MemberDTO타입의 memberId필드값 조건
		...
		*/
		 ...
		 * 서비스에서 HttpSesison이 필요하다면 인자로 전달
		 * 왜냐하면 서비스에서
		 */
