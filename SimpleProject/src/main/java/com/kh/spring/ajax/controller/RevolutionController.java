...
public class RevolutionController {
	 ...
	 * GET /boards/사진/19 => 게시글들 중 사진게시글등 중 19번 게시글 조회
	 * POST /baords    => 새게시글 생성
	 * 
	 * update의 종류
	 * PUT /boards/19	-> 19번게시글 전체  수정
	 * PATCH /boards/19 -> 19번게시글 부분 수정
	 * DELETE /boards/19 => 19번 게시글 삭제
	 * 
	 * 
	 * REST방식을 사용할 때
	 * 서버가 클라이언트한테 응답해줄 때 상태코드를 무조건 보내줘야 함 
	 * 통신이 어떻게 이루어졌다. (404,500,400등)
	 * 
	 * 
	 * HTTP 상태코드 활용
	 * 200OK => 요청이 성공적으로 잘 이루어졌음(GET,DELETE)
	 * 201 Created => 요청에 의한 데이터가 잘 만들어짐(POST,PUT, PATCH)
	 * 400 Bad Request => 잘못된 요청(즉 , 클라이언트의 잘못이다.)
